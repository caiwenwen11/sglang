...
class Scheduler(
    ...
    def get_new_batch_prefill(self) -> Optional[ScheduleBatch]:
        ...
        if self.prefill_delayer:
            # TODO(DSV4) @fzyzcjy: dsv4 fork commit `05ab33bf57 rm token_usage call
            # in prefill delayer` (2026-04-25) hacked this to `token_usage = 0.5`
            # with comment "since it is unused" — assumed prefill_delayer's
            # low_watermark path never reads it under dsv4 prod config. Reverted
            # to main's accurate calculation to align with main #20390 (which
            # extended this with hybrid_swa / hybrid_ssm branches). If prefill
            # scheduling latency regression is observed, re-evaluate the HACK
            # against the actual prod prefill_delayer config.
